...
// Für jede angenommene Verbindung wird ein Kindprozess zur Auftragsverarbeitung gestartet
// (in dieser Implementierung werden Zombieprozesse nicht vermieden):
//
...
int main() {
   ...
   while (1) {
      ...
      if (fork()==0) {
         ...
         exit(0); // Auftrag erledigt
      } else {
         close(consock); // Nicht gebraucht
      }
   }
